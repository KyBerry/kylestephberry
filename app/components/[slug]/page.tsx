...
import { related } from '@/lib/content/helpers'
...
export default async function ShowcaseDetailPage({ params }: PageProps) {
  ...
  const { NotesMDX: Notes, ...frameEntry } = entry
  ...
  return (
    <>
      ...
      {relatedEntries.length > 0 ? (
        <Container variant="grid" as="section" className="border-t border-(--color-border) pt-16 pb-24">
          <h2 className="mb-8 font-mono text-xs uppercase tracking-[0.18em] text-(--color-fg-subtle)">
            Related
          </h2>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {relatedEntries.map(({ NotesMDX: _NotesMDX, ...e }) => (
